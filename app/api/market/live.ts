...
async function getPastPrice(slug: string, hoursAgo: number) {
      ...
      products.map(async (p, i) => {
        try {
          ...
          const matchedItems = items.filter((item: any) => {
            ...
            const hasAllIncludes = p.includeKeywords.every((k) =>
              title.includes(normalise(k))
            );

            const hasExcluded = p.excludeKeywords.some((k) =>
              title.includes(normalise(k))
            );
          ...
          const rawPrices = matchedItems
            .map((item: any) => parseFloat(item.price?.value))
            .filter((v: number) => !isNaN(v) && v > 0);

          const filteredPrices = rawPrices.filter(
            (price) => price >= p.minPrice && price <= p.maxPrice
          );
